...
练习2其实也是相对比较简单的，通过完成FIFO算法来完善虚拟地址空间的物理内存的申请，使用与释放；

当我们用户进程需要申请一个物理内存页时，我们不仅要完善`page directory entry`，`struct Page`等数据结构的相关成员；还需要将该物理内存页应用到page replace algorithm中，由于我们实验中采用的就是简单的FIFO算法，所以我们就将物理页简单地挂在`mm_struct`中的`sm_priv`中；

这里说明一下，由于`list_entry_t`类型是一个双向链表结构，因此顺时针/逆时针方向都有的，所以在这里你认为顺时针方向是一个队列的正常方向或者逆时针方向是一个队列的正常方向都是可以的；只要保证获取到一个物理页时加入到队列尾部，释放一个页面时从队列头部放出就好；

在这里需要说明一下在`_fifo_swap_out_victim`函数中存在的一个注释`set the addr of addr of this page to ptr_page`,这个函数有些许误导性，其实仔细思考一下就可以清楚我们其实是想改变形参`*ptr_page`的指向，让它指向要释放出来的page;
我们知道要在函数中修改一个指针类型的实参就必须将这个指针的地址传入到到函数中，通过这种方式才能都在函数中修改这个函数的指向问题；所以这个注释更准确的应该是`set the addr of addr of this page to ptr_page`;总是这个注释我认为还是十分有**误导性**的！

现在回答一下练习2中的几个问题：


> 问：如果要在ucore上实现"extended clock页替换算法"请给你的设计方案，现有的swap_manager框架是否足以支持在ucore中实现此算法？

答：现有的swap_manager框架是可以支持ucore中实现各种页面置换算法的。

页面置换算法均有如下的特点：
* 在申请新的物理页时加入到自己的数据结构中；
* 在占用的物理页太多时会被替换出去；

而swap_manage框架具有的就是这个功能，那么swap_manager框架如何在编译的过程中使用不同的页面置换算法呢？它是通过将各个函数指针`map_swappalbe`，`set_unswappable`，`swap_out_victim`赋予特定的页面置换算法相应的函数，从而保证这个框架中的关键函数可以按照具体的页面置换算法进行置换，**但是，页面置换算法只是在算法上判定谁应该被置换而何时置换，何时加入这些逻辑都是按照swap_manager框架中进行的**。

> 问：如果是，请给出在ucore上实现"extended clock页替换算法"的设计方案，并回答如下问题：1.需要被换出的页的特征是什么？2.在ucore中如何判断具有这样特征的页？3.何时进行换入和换出操作？

答：这个时钟页面置换算法我会在整体基础题做完后，详细地完成并给出相应的方案，现在先暂时放置在这里，XD。
